#!/bin/bash
echo '========== PHASE 0A =========='
find . -type f -name '*.py' | grep -v 'site-packages' | sort

echo '========== PHASE 0B =========='
find . -type f \( -name '*.csv' -o -name '*.mp4' -o -name '*.avi' -o -name '*.pth' \) | grep -v 'site-packages' | sort
...
echo '========== PHASE 0D =========='
cat src/models/train_production_gw_cd.py 2>&1 || echo "File not found"

echo '========== PHASE 0E =========='
cat src/data/dataset_builder.py 2>&1 || echo "File not found"
