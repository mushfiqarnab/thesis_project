...
UNBIASED_CSV = Path("data/csv/multimodal.csv")
BIASED_CSV = Path("data/csv/multimodal.csv")

# The 6 new strict-split checkpoints
CKPTS = {
    "Lagrangian-Optimized": ("lambda_5.0_final.pth", "cgf", "0"),
}
CKPT_DIR = Path("outputs/lambda_sweep_ckpts")
...
def get_normalization_stats():
    phys_mu = torch.tensor(0.0, device=DEVICE)
    phys_std = torch.tensor(1.0, device=DEVICE)
...
def simple_collate(batch):
    ...
    return {
        ...
        "phys": torch.stack([x["phys"] for x in batch]),
        "y": torch.tensor([x["y"] for x in batch]),
        "scar": torch.tensor([x["scar"] for x in batch]),
        "mask": torch.stack([x["mask"] for x in batch]) if "mask" in batch[0] and batch[0]["mask"] is not None else None
    }

def load_biased_eval_loader():
    ds = MultimodalClinicalDataset(str(BIASED_CSV))
    return DataLoader(ds, batch_size=BATCH, shuffle=False, num_workers=WORKERS, collate_fn=simple_collate)
...
def load_model_securely(ckpt_name: str, fusion: str, disable_stiefel: str) -> torch.nn.Module:
    ...
    model = MultimodalThreatModel(phys_dim=4, fusion=fusion, vision_backbone="mobilenet_v3_small")
...
def main():
    ...
    for name, (ckpt, fusion, disable_stiefel) in CKPTS.items():
        try:
            ...
            if name == "Lagrangian-Optimized" and g_np is not None:
