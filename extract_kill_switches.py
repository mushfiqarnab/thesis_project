...
def main():
    ...
    # Calculate phys norm stats based on the whole dataset (or train split if we were rigorous, but for quick evaluation this is fine)
    ...
    print("Evaluating Standard Multimodal...")
    acc_std, dp_std = evaluate_condition(model, val_loader, device, phys_mu, phys_sigma, 'standard')
    
    print("Evaluating K1 (Physiology-Only / Vision-Dead)...")
    acc_k1, dp_k1 = evaluate_condition(model, val_loader, device, phys_mu, phys_sigma, 'k1')
    
    print("Evaluating K2 (Vision-Only / Physiology-Dead)...")
    acc_k2, dp_k2 = evaluate_condition(model, val_loader, device, phys_mu, phys_sigma, 'k2')
    
    print("\n=== Kill-Switch Baseline Results ===")
    print(f"Standard Multimodal | Accuracy: {acc_std*100:.2f}% | DP Gap: {dp_std:.4f}")
    print(f"K1 (Phys-Only)      | Accuracy: {acc_k1*100:.2f}% | DP Gap: {dp_k1:.4f}")
    print(f"K2 (Vision-Only)    | Accuracy: {acc_k2*100:.2f}% | DP Gap: {dp_k2:.4f}")
