...
% TODO(authors): Confirm that the 1.7149e-6 in the benchmark report was measured on the
% trained checkpoint (the report stores it next to the checkpoint hash); if it was
% measured at initialization, say so instead.
...
\end{itemize}
% TODO(authors): Give the exact surrogate forms of L_DP and L_EO from train_equitas_rcmf.py.
Because the fairness terms are surrogates, a low training loss does not by itself guarantee the corresponding fairness properties~\cite{yao2024}; all fairness criteria are therefore evaluated directly on held-out data. Checkpoints are selected on the validation set by
\begin{equation}
...
% TODO(authors): State which WESAD conditions are used (e.g. stress vs. baseline only, or
% also amusement/meditation as non-stress), and whether X_P is standardized using
...
% TODO(authors): State the number of FFHQ images screened, the number rejected, and the
% scar morphologies used.
...
\end{itemize}
% TODO(authors): State how each regime is constructed from the 496 test samples (for
% example, by reassigning which samples receive the scarred or clean image).
...
\end{table}
% TODO(authors): For each model, state the training CSV and seeds actually used. The
% existing camera-off reports were trained on an earlier dataset
% (multimodal_10k_unbiased.csv), not on this benchmark.
