"""
structural_compressor_v9_apex.py
================================
The Final Edge AI Structural Compressor.
1. Opset 13 Native: Supports edge-friendly aten::diagonal operations natively.
2. Surgical Shielding: Exposes proj/backbone to break the Topological Lock (>30% reduction).
3. Edge NPU QR-Patch: Replaces O(N^3) aten::linalg_qr with an O(N) L2 approximation.
...
logger = logging.getLogger("CompressorV9_Apex")
...
def compress_and_export():
    ...
    logger.info(f"Initializing Apex V9 Compression | Target Prune Ratio: {prune_ratio}")
    ...
    # ── The V9 Exact Class-Type Shield (based on confirmed module tree) ──
    # Import the custom classes to shield them by exact Python type identity
    from src.models.pacd_net import GrassmannProjection
    try:
    ...
    ignored_set = set()
    for name, m in model.named_modules():
        # 1. Shield all custom non-backbone wrapper modules by exact class type
        if isinstance(m, (GrassmannProjection,) + sinkhorn_cls):
            ignored_set.add(m)
        # 2. Shield phys_encoder and classifier Sequential blocks and their children
            ...
            ignored_set.add(m)
        # 3. Shield final classification Linear head
        elif isinstance(m, nn.Linear) and m.out_features == 2:
            ignored_set.add(m)
        # 4. Shield 1D/scalar tensors (BatchNorm, biases) not mathematically shearable
        elif hasattr(m, "weight") and m.weight is not None and m.weight.dim() not in [2, 4]:
            ignored_set.add(m)

    ignored = list(ignored_set)
    logger.info(f"Exact Shield active across {len(ignored)} module(s). Pure backbone Conv2d layers EXPOSED.")
