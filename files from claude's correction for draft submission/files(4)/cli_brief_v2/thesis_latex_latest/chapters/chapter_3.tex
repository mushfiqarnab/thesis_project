...
\end{equation}
so the two blocks project $\mathbf{v}_{\text{raw}}$ onto orthogonal subspaces. The orthogonality error $\|W_{\text{causal}} W_{\text{conf}}^{\top}\|_F$ was measured as $1.7149 \times 10^{-6}$ at initialization; the value for the trained model will be reported in the final version, following recovery of the master checkpoint.
...
% TODO(authors): Define "activation energy" precisely (for example, the squared L2 norm
...
% TODO(authors): Check the code and state (a) which loss trains Focus_auto (for example,
...
\end{equation}
where $\kappa$ is a learnable temperature initialized at $1.5$ and clamped to $[0.1, 10.0]$.
% TODO(authors): State whether G is a scalar or a 192-dimensional vector. The fusion
...
\begin{itemize}
    ...
    \item $D_{\text{JS}} = \operatorname{JS}\bigl(f(X_V, X_P) \,\|\, f(X_V^{S \leftarrow 0}, X_P)\bigr)$ is the Jensen--Shannon divergence between the predicted distributions for an input and its scar-removed counterfactual, with the physiological input held fixed; and
...
The experiments use a pre-registered pilot cohort of eight participants (s1--s8) from UBFC-Phys~\cite{sabour2023}, a multimodal dataset recorded during a social-stress protocol, which provides both the facial video and the wrist-worn BVP and EDA signals used by the model. Each participant contributes three video clips: a rest task (T1), labelled non-stress ($Y = 0$), and speech (T2) and arithmetic (T3) tasks from the Trier Social Stress Test, labelled stress ($Y = 1$). The 24 clips yield 3,344 temporal windows, split into 2,344 training, 504 validation, and 496 test windows.
% TODO(authors): State the window length and stride, which video frame represents each
% window, and which BVP/EDA features make up X_P (e.g. mean IBI, SDNN, RMSSD, EDA
% statistics).
Faces are cropped using a width of $3.566 \times 1.5$ times the inter-ocular distance, a value chosen to reproduce the crop geometry used in the pre-registered extraction. A synthetic brow-line scar is added to the facial frames, with a segmentation mask recorded for each scarred sample, and each scarred frame is paired with a scar-free counterfactual.
% TODO(authors): State how the counterfactual is produced. Because the scar is synthetic,
...
To test whether the model ignores the scar, the association $\rho$ between the scar and the stress label is varied at test time across three regimes:
% TODO(authors): State the scar-label association in the training set. If training uses
...
\begin{itemize}
    \item \textbf{Regime 1 (biased, $\rho = 0.85$):} the scar occurs mostly with stress, matching the association in the training data.
    \item \textbf{Regime 2 (neutral, $\rho = 0.50$):} the scar is distributed equally across classes.
    \item \textbf{Regime 3 (inverted, $\rho = 0.15$):} the scar occurs mostly without stress, reversing that association.
\end{itemize}
Each regime is evaluated in both privileged mode, with the ground-truth mask, and autonomous mode, without it. This draft reports results for the neutral regime in autonomous mode (Chapter~4); results for the biased and inverted regimes will be reported in the final version.
...
\end{table}
% TODO(authors): Confirm in the code that Model C is named CGF (Causal Gated Fusion),
