...
Subgroup accuracies ranged from about 59.7\% to 64.3\%, while the counterfactual gap stayed below 0.001. The low counterfactual gap is the main result, but it must be interpreted together with accuracy: the reported accuracies are below that of the physiology-only model, so it remains to be shown that the fused model uses visual information beyond what physiology already provides (Section~\ref{sec:limitations}). Achieving counterfactual invariance while retaining the benefit of the visual modality is the central open problem for this architecture.
