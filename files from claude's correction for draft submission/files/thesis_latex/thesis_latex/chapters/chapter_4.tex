...
% TODO(authors): The markdown states that EQUITAS-RCMF maintains "consistent
% performance" in all three regimes and both modes, and that the CF gap holds "across
% seeds", but gives no numbers for these claims. Add a table with accuracy, DP, EO and
% CF gap for each regime (rho = 0.85, 0.50, 0.15), each mode, and each seed (mean and
% standard deviation), or remove the claims.
...
\begin{itemize}
    ...
    \item \textbf{Accuracy relative to the physiology-only model.} The subgroup accuracies of EQUITAS-RCMF (59.72--64.29\%) are below the reported accuracy of the camera-off physiology-only model (approximately 72.15\%). If this comparison holds on the same test data, the fused model does not yet benefit from the visual modality, and a small counterfactual gap could partly reflect reduced reliance on visual input in general rather than selective invariance to the scar. Reporting gate values and a vision-only variant of EQUITAS-RCMF would clarify this.
    % TODO(authors): Confirm whether the 72.15% camera-off figure is on the same split
    % and test regime as Table~\ref{tab:demographic_audit}.
